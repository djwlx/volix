...
const buildTreeNode = (item: OpenlistAiOrganizerBrowseItem): TreeNodeData => ({
  key: item.path,
  value: item.path,
  label: (
    <div
...
function SettingOpenlistAiOrganizerApp() {
  const { isAdmin } = useAppPageContext();
  ...
  const loadDirectory = async (targetPath: string) => {
    ...
    return res.data.items.map(item => buildTreeNode(item));
  };
        ...
        <Card
          ...
          headerExtraContent={
            <Space>
              ...
              {currentTaskDetail && ['queued', 'running'].includes(currentTaskDetail.status) ? (
                <Button
                  theme="solid"
                  type="danger"
                  loading={cancelingTaskId === currentTaskDetail.id}
                  onClick={() => handleCancelTask(currentTaskDetail.id)}
                >
                  停止当前任务
                </Button>
              ) : null}
                  ...
                  <Space>
                    ...
                    {['queued', 'running'].includes(record.status) ? (
                      <Button
                        ...
                        onClick={event => {
                          event.stopPropagation();
                          handleCancelTask(record.id).catch(() => undefined);
                        }}
                      >
                        停止
                      </Button>
                    ) : null}
                    {!['failed', 'queued', 'running'].includes(record.status) ? '-' : null}
                  </Space>
                ),
              },
            ]}
          ...
          <Space vertical spacing={12} style={{ width: '100%' }}>
            ...
            >
              <Tree
                ...
                onChange={value => {
                  setSelectedBrowsePath(String(value || '/'));
                }}
                onSelect={selectedKey => {
                  setSelectedBrowsePath(String(selectedKey || '/'));
                }}
