...
import { Button, Card, Empty, Popconfirm, Space, Table, Tag, Toast, Typography } from '@douyinfe/semi-ui';
import {
  ...
  return (
    <div className={styles.page}>
      <Card
        ...
        bodyStyle={{ width: '100%' }}
        headerExtraContent={
          <Button type="primary" onClick={() => requestNavigate('/anime-subscription/add')}>
            新建任务
          </Button>
        }
      >
                ...
                render: (_: unknown, record: AnimeSubscriptionResponse) => (
                  <Space>
                    <Button
                      theme="borderless"
                      type="primary"
                      onClick={() => requestNavigate(`/anime-subscription/edit/${record.id}`)}
                    >
                      编辑
                    </Button>
                    <Button
                    ...
                    <Popconfirm
                      title="确定删除这条追番任务？"
                      content="会同时删除任务条目和运行日志，此操作不可恢复。"
                      okText="删除"
                      okType="danger"
                      position="left"
                      onConfirm={async () => {
                        try {
                          ...
                          const response = await deleteAnimeSubscription(record.id);
                    ...
                    >
                      <Button theme="borderless" type="danger" loading={deletingId === record.id}>
                        删除
                      </Button>
                    </Popconfirm>
                  </Space>
