...
- 媒体工具也内置：格式转换支持本地上传和 OpenList 云文件，并可通过预设或自定义 `ffmpeg` 参数执行转换
...
- 格式转换：支持上传本地媒体文件或选择 OpenList 云文件，使用预设输出方案，或额外填写自定义 `ffmpeg` 参数并跟踪完整任务状态
...
- 格式转换会使用 `data/cache/media/format-convert` 作为临时工作目录，任务完成或启动恢复后会自动清理
